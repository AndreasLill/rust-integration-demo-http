...
pub async fn weather(request: HttpRequest) -> HttpResponse {
    ...
    let response = match HttpClient::new().send(req).await {
        Ok(res) => res,
        Err(_) => return HttpResponse::builder().status(500).body_bytes("Could not fetch geocoding data").unwrap(),
    };

    let body = response.body().as_bytes().await.unwrap();
    let geocoding: GeocodingResponse = match serde_json::from_slice(&body) {
        Ok(res) => res,
        Err(_) => return HttpResponse::builder().status(400).body_bytes("Could not parse weather data").unwrap(),
    };
    ...
    match HttpClient::new().send(req).await {
        Ok(res) => res,
        Err(_) => HttpResponse::builder().status(500).body_bytes("Could not fetch current weather data").unwrap(),
    }
}
